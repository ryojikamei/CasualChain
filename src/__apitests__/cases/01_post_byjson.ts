...
export async function run(conf: any): Promise<number> {
    ...
    const ret5: responseType = await runAxios("/post/byjson", "post", conf.bcapi, payload5);
    if (ret5.data.length !== 24) {
        return -5;
    }
    const payload5_1 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c", key: "desc", value: "test" });
    const ret5_1: responseType = await runAxios("/get/byjson", "get", conf.bcapi, payload5_1);
    if (ret5_1.data === "[]") {
        return -501;
    }
    ...
    const ret6: responseType = await runAxios("/post/byjson", "post", conf.bcapi, payload6);
    if (ret6.data.length !== 24) {
        return -6;
    }
    const payload6_1 = JSON.stringify({ tenant: "8e921d59-00b4-48c2-9ed2-b9f2a90030d6", key: "desc", value: "test" });
    const ret6_1: responseType = await runAxios("/get/byjson", "get", conf.bcapi, payload6_1);
    if (ret6_1.data === "[]") {
        return -601;
    }
    const payload6_2 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c", key: "desc", value: "test" });
    const ret6_2: responseType = await runAxios("/get/byjson", "get", conf.bcapi, payload6_2);
    if (ret6_2.data === "[]") {
        return -602;
    }

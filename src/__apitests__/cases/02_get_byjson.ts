...
export async function run(conf: any, tokens: authTokens): Promise<number> {
    ...
    const ret2: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload2);
    if (ret2.code !== 503) {
        return -2;
    }

    const payload3 = JSON.stringify({ key: "desc", value: "test" });
    const ret3: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload3);
    if (ret3.code !== 503) {
        return -3;
    }

    const payload4 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c", key: "", value: "test" });
    const ret4: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload4);
    if (ret4.data.length !== 0) {
        return -4;
    }

    const payload5 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c", value: "test" });
    const ret5: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload5);
    if (ret5.code !== 400) {
        return -5;
    }

    const payload6 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c", key: "desc", value: "" });
    const ret6: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload6);
    if (ret6.data.length !== 0) {
        return -6;
    }

    const payload7 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c", key: "desc" });
    const ret7: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload7);
    ...
    const ret10: responseType = await runAxios("/get/byjson", "get", conf.bcapi, tokens, payload10);

...
export async function run(conf: any): Promise<number> {
    ...
    const ret1: responseType = await runAxios("/get/byoid/" + "wrong", "get", conf.bcapi);
    if (ret1.code !== 404) {
        return -1;
    }

    const payload2 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c" });
    const ret2: responseType = await runAxios("/get/byoid/" + "012345678901234567890123", "get", conf.bcapi, payload2);
    if (ret2.data._id !== undefined) {
        return -2;
    }

    const ret3: responseType = await runAxios("/get/byoid/" + "012345678901234567890123", "get", conf.bcapi);
    ...
    const ret4_1: responseType = await runAxios("/get/byoid/" + ret4.data, "get", conf.bcapi, payload4_1);

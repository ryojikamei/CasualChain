...
export async function run(conf: any): Promise<number> {
    ...
    // Failed
    const ret1: responseType = await runAxios("/get/pooling", "get", conf.bcapi);
    if (ret1.code !== 503) {
        return -1; 
    }
    ...
    const ret2: responseType = await runAxios("/get/pooling", "get", conf.bcapi, payload2);
    ...
    const ret3_1: responseType = await runAxios("/get/pooling", "get", conf.bcapi, payload3_1);

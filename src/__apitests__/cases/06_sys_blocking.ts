...
export async function run(conf: any, tokens: authTokens): Promise<number> {
    ...
    const ret3_3: responseType = await runAxios("/get/blocked", "get", conf.bcapi, tokens, payload3_3);
    if (ret3_3.data.length <= 1) {
        return -5;
    }
    const payload3_4 = JSON.stringify({ tenant: "a24e797d-84d1-4012-ba78-8882f2711f6c" });
    const ret3_4: responseType = await runAxios("/get/pooling", "get", conf.bcapi, tokens, payload3_4);

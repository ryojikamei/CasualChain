...
export async function run(conf: any, tokens: authTokens): Promise<number> {
    ...
    const ret1_1: responseType = await runAxios("/get/history/" + ret1.data, "get", conf.bcapi, tokens, payload0);
    if (ret1_1.data.length !== 1) {
        return -2;
    }

    // Failed
    const ret2: responseType = await runAxios("/get/history/" + ret1.data, "get", conf.bcapi, tokens);
    if (ret2.code !== 503) {
        return -3;
    }
    ...
    const ret3: responseType = await runAxios("/get/history/" + "wrong", "get", conf.bcapi, tokens, payload0);
    if (ret3.code !== 404) {
        return -4;
    }
    const ret3_1: responseType = await runAxios("/get/history/" + "012345678901234567890123", "get", conf.bcapi, tokens, payload0);
    ...
    const ret4_1: responseType = await runAxios("/post/byjson", "post", conf.bcapi, tokens, payload4_1);
    ...
    const ret4_2: responseType = await runAxios("/post/byjson", "post", conf.bcapi, tokens, payload4_2);
    if (typeof(ret4_2.data) !== "string") {
        return -7;
    }

    const ret4_3: responseType = await runAxios("/get/history/" + ret4_2.data, "get", conf.bcapi, tokens, payload0);

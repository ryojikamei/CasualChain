...
export async function run(conf: any): Promise<number> {
    ...
    // Failed
    const ret1: responseType = await runAxios("/get/lastblock", "get", conf.bcapi);
    if (ret1.code !== 503) {
        return -1;
    }
    
    const ret2: responseType = await runAxios("/get/lastblock", "get", conf.bcapi, payload0);
    ...
    const ret3_2: responseType = await runAxios("/sys/blocking", "post", conf.bcapi, payload0);
    const ret3_3: responseType = await runAxios("/get/lastblock", "get", conf.bcapi, payload0);

...
export async function run(conf: any): Promise<number> {
    ...
    // Failed
    const ret1: responseType = await runAxios("/get/totalnumber", "get", conf.bcapi);
    if (ret1.code !== 503) {
        return -1;
    }

    const ret2: responseType = await runAxios("/get/totalnumber", "get", conf.bcapi, payload0);
    if (ret2.data !== 4) {
        return -2;
    }
    ...
    const ret3_3: responseType = await runAxios("/get/totalnumber", "get", conf.bcapi, payload0);
    if (ret3_3.data !== 5) {
        return -5;
    }

...
export async function run(conf: any, tokens: authTokens): Promise<number> {
    ...
    // Failure
    const payload1 = "{}";
    const ret1: responseType = await runAxios("/sys/closetenant", "post", conf.bcapi, tokens, payload1);
    if (ret1.code !== 400) {
        return -1;
    }
    const payload2 = JSON.stringify({ "adminId": "8e921d59-00b4-48c2-9ed2-b9f2a90030d6" });
    const ret2: responseType = await runAxios("/sys/closetenant", "post", conf.bcapi, tokens, payload2);
    if (ret2.code !== 400) {
        return -2;
    }
    const payload3 = JSON.stringify({ "adminId": "8e921d59-00b4-48c2-9ed2-b9f2a90030d6", "tenantId": "wrong" });
    const ret3: responseType = await runAxios("/sys/closetenant", "post", conf.bcapi, tokens, payload3);
    if (ret3.code !== 503) {
        return -3;
    }
    ...
    const ret5: responseType = await runAxios("/sys/closetenant", "post", conf.bcapi, tokens, payload5);
    if (ret5.code !== 200) {
        return -5;
    }
